...
# ╚══════════════════════════════════════════════════════════════════╝
def render_hero():
    # Inline SVGs with leading/trailing newlines confuse Streamlit's markdown
    # parser: </svg> is treated as a block boundary and all HTML after it is
    # escaped as raw text.  Encoding as a base64 <img> avoids this entirely —
    # <img> is a void element and cannot break the surrounding HTML structure.
    _svg_bytes = dna_helix_svg(width=180, height=60).strip().encode("utf-8")
    _b64       = base64.b64encode(_svg_bytes).decode("utf-8")
    _svg_img   = (
        f'<img src="data:image/svg+xml;base64,{_b64}" '
        f'width="180" height="60" style="display:block"/>'
    )
    st.markdown(
        f"""
