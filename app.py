...
# ╚══════════════════════════════════════════════════════════════════╝
def render_hero():
    # SVG encoded as base64 <img> — void element cannot break surrounding HTML.
    _svg_bytes = dna_helix_svg(width=190, height=68).strip().encode("utf-8")
    _b64       = base64.b64encode(_svg_bytes).decode("utf-8")
    _svg_img   = (
        f'<img src="data:image/svg+xml;base64,{_b64}" '
        f'width="190" height="68" style="display:block;filter:drop-shadow(0 0 8px rgba(96,165,250,0.5))"/>'
    )
    st.markdown(
        f"""
        <style>
        @keyframes heroGlow {{
            0%,100% {{ box-shadow:0 4px 24px rgba(59,130,246,0.18),0 0 0 1px rgba(59,130,246,0.12); }}
            50%      {{ box-shadow:0 6px 40px rgba(59,130,246,0.40),0 0 0 1px rgba(59,130,246,0.35); }}
        }}
        @keyframes livePulse {{
        ...
        .hero-wrap {{
            background: linear-gradient(135deg,{PRIMARY_DARK} 0%,{PRIMARY} 55%,#2D5384 100%);
            padding: 30px 34px;
            border-radius: 16px;
            border-left: 5px solid {DANGER_RED};
            margin-bottom: 18px;
            animation: heroGlow 3.5s ease-in-out infinite, borderPulse 3.5s ease-in-out infinite;
            position: relative;
            overflow: hidden;
        }}
        .hero-wrap::before {{
            ...
            <div style="display:flex;justify-content:space-between;align-items:center">
                <div style="flex:1">
                    ...
                    <div class="hero-title">
                        Lung Cancer Detection<br>from Serum miRNA Expression
                    </div>
                    <div style="margin-top:12px">
                        <span class="hero-tag" style="color:#93C5FD;border-color:rgba(147,197,253,0.35);
                              background:rgba(147,197,253,0.1);animation-delay:0.05s">
                            🧬 10-miRNA Panel
                        </span>
                        <span class="hero-tag" style="color:#86EFAC;border-color:rgba(134,239,172,0.35);
                              background:rgba(134,239,172,0.1);animation-delay:0.15s">
                            📊 AUC 99.97%
                        </span>
                        <span class="hero-tag" style="color:#FCD34D;border-color:rgba(252,211,77,0.35);
                              background:rgba(252,211,77,0.1);animation-delay:0.25s">
                            ⚡ Platt-Calibrated
                        </span>
                        <span class="hero-tag" style="color:#F9A8D4;border-color:rgba(249,168,212,0.35);
                              background:rgba(249,168,212,0.1);animation-delay:0.35s">
                            🔍 SHAP Explainable
                        </span>
                    </div>
                </div>
                <div class="helix-wrap" style="margin-left:28px;flex-shrink:0">
                    {_svg_img}
                </div>
            </div>
        </div>
...
def main():
    ...
    st.markdown(
        f"""
        <div style="margin-top:32px;padding:14px 20px;text-align:center;
                    background:{PRIMARY_DARK};border-radius:10px;
                    border:1px solid rgba(147,197,253,0.12)">
            <div style="color:#94A3B8;font-size:11px;line-height:1.6">
                ⚠️ <b style="color:#CBD5E1">Not for clinical decision-making</b> ·
